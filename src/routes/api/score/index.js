...
export async function GET(request){
    const client = new MongoClient(uri); //client 객체생성
...
export async function PUT(request){ // put 함수는 브라우저에서 넘어오는 데이터를 파싱해서 score 컬렉션에 삽입
    const client = new MongoClient(uri);    //client 객체 생성
    let rtn = {};                           // 결과 저장 객체
    // let score = await JSON.parse(request.body);   // request body를 객체로 변환
    let score = await request.request.json(); // request의 json 객체를 받아오기
