<script>
    ...
    function next(){
        idx = idx+1;
        if(idx==list.length){   // 더 이상 문제가 없으면 stop 호출
            stop();
        }else{
            current=list[idx]; // list 에서 새로운 문제 꺼냄
            current.choice.sort(()=>Math.random()-0.5); // 선택지 랜덤으로 섞음
            current=current; // 리액티비를 위한 할당
            resetProgress(); // 프로그레스 리셋 함수
        }
    }

    function resetProgress(){
        clearInterval(timer); // 이전 타이머 초기화
        bonus=100; // 보너스 100으로 시작
        timer=setInterval(function(){
            bonus=bonus-1 // 보너스 1점씩 감소
            if(bonus==0){ // 보너스 점수가 0이 되면 타이머 초기화
                clearInterval(timer);
            }
        },40) // 1점 감소 주기 40밀리 초
    }
...
{:else}
    ...
    {:else}
        ...
        <p></p>
        {#each current.choice as item}
        <button style="color:white;background-color:{item.color};" class="choice" on:click={()=>{$score=$score+(current.question.color==item.text ? 10+bonus:0);next()}}>
            {kolor[item.text]}
        </button>
        {/each}
    {/if}
